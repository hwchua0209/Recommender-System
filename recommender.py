...
class Content_filtering:
    ...
    def bag_of_words(self):
        ...
        data = self.df.groupby(self.df.title)['genres'].unique().reset_index()    
...
class Colab_filtering:
    ...
    def recommend(self, userId):
        ...
        df_prediction = pd.DataFrame(predicted_list, columns=['movieId', 'Predicted_rating'])
        predict = df_prediction.sort_values(by='Predicted_rating', ascending=False)['movieId'][:5].values
        recommended_movie = []
        for x in predict:
            recommended_movie.append(self.df.title[self.df.movieId == x].unique()[0])
...
class Hybrid:
    ...
    def recommend(self, userId):
        ...
        reader = Reader()
        svd_df = Dataset.load_from_df(data[['userId', 'movieId', 'rating']], reader)
